...
public final class RecordAccumulator {
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            // 获取或创建 TopicPartition 的双端队列
            Deque<ProducerBatch> dq = getOrCreateDeque(tp);
            synchronized (dq) {
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                // 将当前消息添加到队列中，如果添加成功则返回
     ...
     *
     *  如果队列是 null，则返回 null 并创建新的批次；同时也会关闭批次释放资源，如压缩缓冲；
     *  如果有以下场景，则批次将被完全关闭：发送之前过期、producer 被关闭
     */
     ...
     * partition batches.
     * 获取 Partition 可以发送消息的 Node 的集合，以及不可以发送的最早时间，同时返回 Partition 是否有未知的 leader
     *
     * <p>
            ...
            synchronized (deque) {
                ...
                if (batch != null) {
                    ...
                    if (leader == null) {
                        ...
                        // 如果 leader 不是 ready 或者没有被静音
                        long waitedTimeMs = batch.waitedTimeMs(nowMs);
                        boolean backingOff = batch.attempts() > 0 && waitedTimeMs < retryBackoffMs;
                        long timeToWaitMs = backingOff ? retryBackoffMs : lingerMs;
                        boolean full = deque.size() > 1 || batch.isFull();
                        boolean expired = waitedTimeMs >= timeToWaitMs;
                        // 如果队列已满、超过等待期限、资源耗尽、关闭、强制发送
                        boolean sendable = full || expired || exhausted || closed || flushInProgress();
                        // 如果上述情况，且没有退避
                        if (sendable && !backingOff) {
                            // 将 leader 节点加入 ready 节点
     ...
     * 将所有数据发送给指定节点，并将它们添加到批次集合中
     * 这个方法试图避免一次又一次的选择相同的节点
     *
     ...
     * 根据 TopicPartition 获取双端队列，如果不存在则创建一个
     */
